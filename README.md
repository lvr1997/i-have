...
启动前台学生端项目，在项目目录下运行 `pnpm run dev` 启动项目，默认端口为 `http://localhost:8080`

本项目使用 `PNPM` 作为前端项目的包管理工具，你需要先安装PNPM ，然后在项目目录下运行 `cd ihave-ui` 进入前台学生端项目目录，运行 `pnpm install` 安装项目依赖。
